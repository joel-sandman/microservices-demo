...
def plot_network_traffic():
    ...
    fine_grained_network_requests = get_network_traffic_by_requests('fine-grained', '*')
    full_page_network_requests = get_network_traffic_by_requests('full-page', '*')
    
    plt.plot(ttls, fine_grained_network_requests[1:], 'b-.', label='Fine-grained caching')
    plt.plot(ttls, full_page_network_requests[1:], 'r--', label='Full-page caching')
    plt.xscale('log')
    plt.xticks(ttls, ttls)
    plt.axis([0.075, 25, 0, 3800])
    plt.legend()
    plt.show()
...
def plot_memory_usage():
    ...
    plt.plot(ttls, fine_grained_memory_usage, 'b-.', label='Fine-grained caching')
    plt.plot(ttls, full_page_memory_usage, 'r--', label='Full-page caching')
    plt.plot(ttls, [0]*len(ttls), 'g-', label='No caching')
    plt.xscale('log')
    plt.xticks(ttls, ttls)
    plt.xlabel('TTL (s)')
